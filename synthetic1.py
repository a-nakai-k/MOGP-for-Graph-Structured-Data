...
def signal_generator(p, q, r, t):
    return p * np.cos(q * t) + r * t
...
def log_likelihood_base(alpha_log = 1., beta_log = 1., lengthscale = 10., variance = np.var(yn), noise = 0.05):
    ...
    if parser.model in ('regularized_laplacian', 'diffusion', '1_random_walk', '3_random_walk', 'cosine', 'reg_diff', 'matern2', 'matern3', 'matern5', 'reg_matern3'):
        ...
        CgN = np.kron(np.matmul(B1,B1.T), K) + np.kron(B2, K) + (noise * np.eye(M*N))
    else:
...
if __name__ == '__main__':
    print('data: synthetic1, training data: {}, model: {}, kernel: {}, k: {}'.format(N, parser.model, parser.kernel, parser.k), flush=True)
    ...
    for i in range(1,num_test+1):
        ...
        if parser.model in ('regularized_laplacian', 'diffusion', '1_random_walk', '3_random_walk', 'cosine', 'reg_diff', 'matern2', 'matern3', 'matern5', 'reg_matern3'):
            ...
            CgN = np.kron(np.matmul(B1,B1.T), K) + np.kron(B2, K) + (noise * np.eye(M*N))
            D = np.kron(np.matmul(B1,B1.T), k) + np.kron(B2, k)
            F = np.kron(np.matmul(B1,B1.T), k_star) + np.kron(B2, k_star) + (noise * np.eye(M*MM))
        else:
            CgN = np.kron(np.matmul(B,B.T), K) + (noise * np.eye(M*N))
